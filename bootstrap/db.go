...
func initSqliteGorm() *gorm.DB {
	dbConfig := global.App.Config.Database
	fmt.Println("sqlite path:" + dbConfig.Path)
	if db, err := gorm.Open(sqlite.Open(dbConfig.Path), &gorm.Config{
		DisableForeignKeyConstraintWhenMigrating: true,            // 禁用自动创建外键约束
		Logger:                                   getGormLogger(), // 使用自定义 Logger
	}); err != nil {
		fmt.Println("connect sqlite success" + err.Error())
		global.App.Log.Error("sqlist connect failed, err:", zap.Any("err", err))
		return nil
	} else {
		//sqlDB, _ := db.DB()
		initSqliteTables(db)
		fmt.Println("connect sqlite success")
		return db
	}
}
